...
battery_full(const char *bat)
{
    ...
    // read capacity
    snprintf(buf, sizeof(buf), "/sys/class/power_supply/%s/capacity", bat);
    fd = fopen(buf, "r");
    if (!fd) return "N/A";
    if (fscanf(fd, "%d", &perc) != 1) { fclose(fd); return "N/A"; }
    fclose(fd);

    // read state
    snprintf(buf, sizeof(buf), "/sys/class/power_supply/%s/status", bat);
    fd = fopen(buf, "r");
    if (!fd) return "N/A";
    if (!fgets(state, sizeof(state), fd)) { fclose(fd); return "N/A"; }
    fclose(fd);
    state[strcspn(state, "\n")] = 0;
